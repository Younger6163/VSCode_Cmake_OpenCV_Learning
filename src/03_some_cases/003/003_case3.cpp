...
int main(int argc, char** argv) {
	...
	for (size_t t = 0; t < contours.size(); t++) {
		...
		if (ratio < 1.1 && ratio > 0.9) {
			drawContours(resultImage, contours, t, Scalar(0, 255, 0), -1, 8, Mat(), 0, Point());
			...
			Point2f cc_fit;
			float radius_fit = 0.0;
			minEnclosingCircle(contours[t], cc_fit,radius_fit);
			circle(resultImage, cc_fit, radius_fit, Scalar(255, 0, 0), 2, 8, 0);
		}
	}
	...
	// HoughCircles(gray_result, myCircles, HOUGH_GRADIENT, 1, 7, 50, 20, 10, 100);

	// Mat circleImage = src.clone();
	// cvtColor(circleImage, circleImage, COLOR_GRAY2BGR);
	// for (int i = 0; i < myCircles.size(); i++) {
	// 	Vec3f circleInfo = myCircles[i];
	// 	circle(circleImage, Point(circleInfo[0], circleInfo[1]), circleInfo[2], Scalar(255, 0, 0), 1, 8, 0);
	// }
